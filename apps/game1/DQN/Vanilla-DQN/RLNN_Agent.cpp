...
int RLNN_Agent::selectAction(observation &currentState, int episodeCount, bool *explore) {
    ...
    *explore = isTrainingMode and isExplore(episodeCount);
    if (*explore) {
        ...
        auto tensor_path = torch::from_blob(pathFOV, {1, FOV_WIDTH, FOV_WIDTH}, options).unsqueeze(1).clone();
        auto tensor_fov_channels= torch::cat({tensor_path, tensor_obstacles, tensor_enemies}, 1).clone();
...
double RLNN_Agent::learnWithDQN() {
    ...
    ///*
    if (loss <= 1 and ((episodeNumber >= 4000) and (episodeNumber % 1000 == 0))) {
